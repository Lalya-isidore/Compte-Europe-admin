...
<head>
    <title>Diminution de Solde sur {{ config('app.name') }}</title>
    <style>
        body {
...
<body>
    <div class="container">
        ...
        <div class="content">
            ...
            <p>Nous souhaitons vous informer que votre solde a été diminué de {{ number_format($montant, 2, ',', ' ') . ' ' . $compte->devise }}.</p>
            <p>Voici les détails de l'opération :</p>
            <ul>
                <li><strong>Montant déduit :</strong> {{ number_format($montant, 2, ',', ' ') . ' ' . $compte->devise }}</li>
